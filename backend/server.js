...
app.post('/api/users/register', async (req, res) => {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO users (id, email, password, firstName, lastName, phone, address)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(
      userId,
    ...
    res.status(201).json({
      token,
      user: { id: userId, email: emailKey, firstName, lastName, phone, address }
    });
  } catch (error) {
...
app.get('/api/users/me', (req, res) => {
  ...
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    const stmt = db.prepare('SELECT id, email, firstName, lastName, phone, address FROM users WHERE id = ?');
...
app.get('/api/public/orders/:id', (req, res) => {
  try {
    const stmt = db.prepare('SELECT id, firstName, lastName, payment, items, total, status, createdAt FROM orders WHERE id = ?');
